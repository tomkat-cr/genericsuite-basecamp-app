...
# def translate_text(first_text, dest_lang):
#     response = {'error': '', 'output_text': ''}
#     try:
#         print("Init translator...")
#         translator = Translator()
#     except Exception as err:
#         response['error'] = f"ERROR [TT-010]: {str(err)}"
#     if not response['error']:
#         # Translate text to target language
#         try:
#             print(f"Translator.translate to: {dest_lang}")
#             output_text = translator.translate(first_text, dest=dest_lang)
#             print(f"Text translated: {output_text.text}")
#             response['output_text'] = output_text.text
#         except Exception as err:
#             response['error'] = f"ERROR [TT-020]: {str(err)}"
#     return response
